...
function sys_del_attach(id,fun){
	 autoAjax("/base/attach/del",{id:id} ,function(data){
		 if(fun &&typeof fun == 'function'){
			 fun(data);
		 }
	 });
	...
	var oldnums = window.sessionStorage.getItem("oldnums");
	 ...
	 var num = {};
	    ...
	    if(oldnums==newnums){
	    	...
	    	autoAjax("/base/public/repeatsort",{newnums:newnums,dbtable:dbtable},function(json){	    	
		    			   if(json.result==true){
		    				   sys_alert(json.msg);
					         }
	   		  });
		...
		if(value==undefined){
			...
			for(var key in objMsg){
				if(value==''||value==null){
					//必填
		    		if(key=='required'){
    				sys_alert(objMsg['required'],function(){$cs.focus();});
    				ret_val=false;
    				return false;
		    			}
		    		}else{
		    			...
		    			case"min":
		    			  ...
		    			  if(!ret_val){
		    				...
		    				sys_alert(str_msg,function(){$cs.focus();});
		    				ret_val=false;
		    				return false;
		    			  }
